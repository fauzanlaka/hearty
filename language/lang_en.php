<?php
    $lang_text = array(
        'explain' => 'explain',
        'Narrative Account of Alleged Torture and Ill Treatment' => 'Narrative Account of Alleged Torture and Ill Treatment',
        'At this point, let’s talk about what happened to when you were arrested. It will be helpful if you tell me first about the arrest and then what happened to you, step by step during the time that you were detained. Please include as much detail as you can and let me know if there is something that you are not sure about' => 'At this point, let’s talk about what happened to when you were arrested. It will be helpful if you tell me first about the arrest and then what happened to you, step by step during the time that you were detained. Please include as much detail as you can and let me know if there is something that you are not sure about',
        'Review of Torture Methods' => 'Review of Torture Methods',
        'Blunt trauma' => 'Blunt trauma (check and Describe)',
        'punch' => 'punch',
        'kick' => 'kick',
        'slap' => 'slap',
        'whips' => 'whips',
        'wires' => 'wires',
        'truncheons' => 'truncheons',
        'falling down' => 'falling down',
        'Frequency' => 'frequency',
        'Duration' => 'duration',
        'Restraint/Position' => 'restraint/Position',
        'Injury Location' => 'injury location(s)',
        'Injury Observations' => 'injury observations',
        'Head Trauma' => 'head trauma (check and describe)',
        'bruises' => 'bruises',
        'bleeding' => 'bleeding',
        'loss of consciousness' => 'loss of consciousness',
        'numbness' => 'numbness',
        'weakness' => 'weakness',
        'etc' => 'etc',
        'Positional torture' => 'Positional torture (check and describe)',
        'Suspension' => 'suspension',
        'stretching limbs apart' => 'stretching limbs apart',
        'prolonged constraint of movement' => 'prolonged constraint of movement',
        'forced positioning' => 'forced positioning',
        'being chained or tied' => 'being chained or tied',
        'placed in a small space' => 'placed in a small space (e.g., a box or under a chair)',
        'forced to stand' => 'forced to stand',
        'lie, or kneel in an uncomfortable position' => 'lie, or kneel in an uncomfortable position',
        'can' => 'yes',
        'can not' => 'no',
        'Exposure to extreme temperatures' => 'exposure to extreme temperatures (check and describe)',
        'exposed to heat' => 'exposed to heat',
        'sun' => 'sun',
        'rain' => 'rain',
        'body immersion' => 'body immersion',
        'cold' => 'cold',
        'forced to sit on ice' => 'forced to sit on ice',
        'Burns' => 'Burns (check and describe)',
        'burn cigarettes' => 'burn cigarettes',
        'heated instrument' => 'heated instrument',
        'scalding water or other liquid' => 'scalding water or other liquid',
        'caustic substance' => 'caustic substance',
        'Electric shock' => 'electric shock',
        'describe' => 'describe',
        'Asphyxiation' => 'Asphyxiation (check and describe)',
        'wet and dry methods' => 'wet and dry methods',
        'drowning' => 'drowning',
        'smothering' => 'smothering',
        'choking chemicals' => 'choking chemicals',
        'Crush injuries' => 'crush injuries (check and Describe)',
        'smashing fingers' => 'smashing fingers',
        'heavy roller to thighs/back' => 'heavy roller to thighs/back',
        'Physical evidence' => 'Physical evidence',
        'Acute Injury Type (select)' => 'Acute Injury Type (select)',
        'Pain' => 'pain',
        'Other' => 'other',
        'specify' => 'specify',
        'Laceration' => 'Laceration',
        'Contusion' => 'Contusion',
        'Abrasion' => 'Abrasion',
        'Incision' => 'Incision',
        'Penetrating'=> 'Penetrating',
        'Bleeding' => 'Bleeding',
        'Loss of Consciousness' => 'Loss of Consciousness',
        'Broken Bone' => 'Broken Bone',
        'Deformity' => 'Deformity',
        'Location' => 'Location',
        'Intensity' => 'Intensity',
        'Physical Symptoms and Disabilities' => 'Physical Symptoms and Disabilities',
        '> 6 Weeks' => '>6 Weeks',
        'yes1' => 'yes',
        'No' => 'No',
        'Healing Time' => 'Healing Time',
        'Current' => 'Current',
        'physical scars or marks' => 'Do you have any physical scars or marks on your body now that you think are related to the experiences you described?',
        'Where on your body' => 'Where on your body? (specify)',
        'Mental Health Post-Detention' => 'Medical/Mental Health Care During and Post-Detention' ,
        'Did you receive any medical examination' => 'Did you receive any medical examination',
        'Before, during or upon release from detention?' => 'Before, during or upon release from detention?',
        'HaveYes' => 'yes',
        'HaveNo' => 'no',
        'YesUsedto' => 'yes',
        'NoUsedto' => 'No',
        'After your release?' =>  'After your release?',
        'receive any medical treatment' => 'Did you receive any medical treatment for physical or mental health problems',
        'Penetrating injuries' => 'penetrating injuries (check and Describe)',
        'stab and gunshot wounds' => 'stab and gunshot wounds',
        'wires needles under nails' => 'wires needles under nails',
        'toes in sensitive parts of the body' => 'toes in sensitive parts of the body',
        
        'Chemical exposures' => 'Chemical exposures (in wounds, body cavities) (check and describe)',
        'gasoline' => 'gasoline',
        'salt' => 'chili',
        'chili' => 'chili'
    );
?>